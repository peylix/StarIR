...
parser.add_argument('--data', default='GOPRO', choices=['GOPRO', 'HIDE', 'RealBlur-R', 'RealBlur-J'], type=str, help='dataset')               
parser.add_argument(
...
if args.data == 'HIDE':
    weights = os.path.join(args.weights, 'StarIR-GOPRO.pth')

    opt_path = os.path.join(args.opt, 'GOPRO.yml')
else:
...
if args.data in ['GOPRO', 'RealBlur-J', 'RealBlur-R']: 
    opt['network_g']['basesize'] = 95
elif args.data == 'HIDE':
...
input_dir = os.path.join(args.input_dir, args.data, 'valid', 'blur')
