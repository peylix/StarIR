...
def main(args):
    ...
    iqa_lpips = pyiqa.create_metric('lpips-vgg')
    iqa_lpips.eval() 
    ...
    for i, img_path in enumerate(img_list_gt):
        ...
        img_restored_lpips = np.transpose(img_restored, (2, 0, 1))
        img_restored_lpips = torch.from_numpy(img_restored_lpips).float()

        img_gt_lpips = np.transpose(img_gt, (2, 0, 1))
        img_gt_lpips = torch.from_numpy(img_gt_lpips).float()
        with torch.no_grad():
            img_restored_lpips = img_restored_lpips.unsqueeze(0)
            img_gt_lpips = img_gt_lpips.unsqueeze(0)
            lpips = iqa_lpips(img_restored_lpips, img_gt_lpips).item()
        ...
        print(f'{i+1:3d}: {basename:25}. \tPSNR: {psnr:.6f} dB, \tSSIM: {ssim:.6f}, \tLPIPS: {lpips:.6f}')
        psnr_all.append(psnr)
        ssim_all.append(ssim)
        lpips_all.append(lpips)
    print(args.gt)
    print(args.restored)
    print(f'Average: PSNR: {sum(psnr_all) / len(psnr_all):.6f} dB, SSIM: {sum(ssim_all) / len(ssim_all):.6f}, LPIPS: {sum(lpips_all) / len(lpips_all):.6f}')


if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('--gt', type=str, default='./Datasets/LOL-Blur/Test/high_sharp_scaled_all', help='Path to gt (Ground-Truth)')
    parser.add_argument('--restored', type=str, default='./results/LOL-Blur', help='Path to restored images')
