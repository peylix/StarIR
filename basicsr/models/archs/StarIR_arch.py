...
class StarModule(nn.Module):
    def __init__(self, dim, bias, basesize=None):
        ...
        # work when generating images
        if basesize:
            gap = AvgPool2d(base_size=basesize)
        else:
...
class StarBlock(nn.Module):
    def __init__(self, dim, ffn_expansion_factor=2.66, bias=False, LayerNorm_type='WithBias', basesize=None):
        super(StarBlock, self).__init__()

        self.norm1 = LayerNorm(dim, LayerNorm_type)
        self.attn = StarModule(dim, bias, basesize=basesize)
...
class StarIR(nn.Module):
    def __init__(self,
        ...
        self.encoder_level1 = nn.Sequential(*[
            StarBlock(dim=dim, ffn_expansion_factor=ffn_expansion_factor, bias=bias, basesize=basesize) 
            for i in range(num_blocks[0])])

        self.down1_2 = Downsample(dim)

        self.encoder_level2 = nn.Sequential(*[
            StarBlock(dim=int(dim * 2 ** 1), ffn_expansion_factor=ffn_expansion_factor, bias=bias, basesize=basesize) 
            for i in range(num_blocks[1])])

        self.down2_3 = Downsample(int(dim * 2 ** 1))

        self.encoder_level3 = nn.Sequential(*[
            StarBlock(dim=int(dim * 2 ** 2), ffn_expansion_factor=ffn_expansion_factor, bias=bias, basesize=basesize) 
        ...
        self.decoder_level1 = nn.Sequential(*[
            StarBlock(dim=int(dim), ffn_expansion_factor=ffn_expansion_factor, bias=bias, basesize=basesize) 
            for i in range(num_blocks[0])])
