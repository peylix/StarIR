...
parser = argparse.ArgumentParser(description='Single Image Defocus Deblurring using StarIR')

parser.add_argument('--input_dir', default='./Datasets/test/', type=str, help='Directory of validation images')
parser.add_argument('--result_dir', default='./results/dpdd-single/', type=str, help='Directory for results')
parser.add_argument('--weights', default='./pretrained_models/StarIR-DPDD-Single.pth', type=str, help='Path to weights')
parser.add_argument('--save_images', action='store_true', help='Save denoised images in result directory')

args = parser.parse_args()

####### Load yaml #######
yaml_file = './Options/DPDD-Single.yml'
import yaml
...
with torch.no_grad():
    for fileI, fileC in tqdm(zip(filesI, filesC), total=len(filesC)):
        ...
        b, c, h, w = patchI.shape
        H, W = ((h + factor) // factor) * \
            factor, ((w + factor) // factor) * factor
        padh = H - h if h % factor != 0 else 0
        padw = W - w if w % factor != 0 else 0
        patchI = F.pad(patchI, (0, padw, 0, padh), 'reflect')


        restored = model_restoration(patchI)[:, :, :h, :w]
